...
export async function setupSupabase(progressBar = null, projectRoot = null) {
  ...
  const migrateProfile = (p) => {
    if (p.SUPABASE_BACKEND_URL && !p.SUPABASE_INFRA_URL) {
      p.SUPABASE_INFRA_URL = p.SUPABASE_BACKEND_URL
    }
  ...
  const hasInfraUrl = existingProfile?.SUPABASE_INFRA_URL || existingProfile?.SUPABASE_BACKEND_URL
  ...
  if (existingProfile && existingProfile.USERNAME && hasInfraUrl) {
    ...
    infraDbUrl = existingProfile.SUPABASE_INFRA_URL || existingProfile.SUPABASE_BACKEND_URL
      ...
      const { infraUrl } = await inquirer.prompt([
        {
          ...
          default: profile?.SUPABASE_INFRA_URL || profile?.SUPABASE_BACKEND_URL || DEFAULTS.infraDb.url,
          validate: (input) => {
            if (!input || input.trim().length === 0) {
